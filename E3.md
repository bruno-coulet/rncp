...
L'accès au modèle est strictement protégé. J'ai implémenté un Middleware d'authentification exigeant la présence d'une `X-API-Key` dans les en-têtes HTTP. Conformément aux recommandations OWASP (Broken Authentication), les clés API sont hachées en base de données PostgreSQL. De plus, des "garde-fous" sanitaires (règles métiers OMS) sont codés en dur dans l'API : si un prélèvement présente un pH < 6.5 ou une turbidité > 5.0 NTU, l'API rejette l'échantillon immédiatement sans solliciter le modèle.
...
L'API Drink Safe fait appel au service Cloud **OCR.space** de manière asynchrone. La route `POST /api/ocr/lab-report` reçoit le fichier (PDF ou Image) de l'utilisateur, l'envoie de manière sécurisée à l'API OCR, puis "parse" le retour JSON via des expressions régulières (Regex) pour extraire les 9 mesures physico-chimiques requises.
