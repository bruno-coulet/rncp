...
Afin de garantir la sécurité et d'optimiser le poids de l'infrastructure, le conteneur Docker de production de l'API est strictement dépourvu de tout outil de développement ou de test (comme pytest). L'exécution des tests et la mesure de la couverture sont réalisées exclusivement dans des environnements isolés ou éphémères.
