...
## Intégrer des modèles et des services d’intelligence artificielle  - veille, benchmark et paramétrage
...
![Capture de l'interface Inoreader montrant les flux RSS classés par thèmes comme la cybersécurité et l'IA](img/img_E2/E2_C6_inoreader_feed.png)
...
<blockquote>à partir de l’expression de besoin en fonctionnalités d’intelligence artificielle, en réalisant un benchmark de services existants et en analysant leurs caractéristiques pour formaliser une ou plusieurs recommandations de services adaptés au besoin.</blockquote>
...
Pour pallier la contrainte de dépendance réseau, je prévois d'implémenter un mécanisme de secours (fallback gracieux, voir le document **E4.pdf** au chapitre C 17) dans le code pour gérer les potentiels temps d'arrêt de l'API externe.
...
<blockquote>en suivant sa documentation technique et en respectant les spécifications du projet, afin de permettre l’intégration des connecteurs du service dans le système d’information.</blockquote>
...
Le paramétrage de l'API tierce a été optimisé pour le besoin métier en suivant la [documentation officielle de l'API OCR.space](https://ocr.space/ocrapi):
`language='fre'`<br>
...
À la réception du JSON renvoyé par l'IA, l'API métier exécute ses algorithmes d'expressions régulières (détaillés dans l'annexe **parsing_ocr.pdf**) pour extraire les mesures.
...
La logique complète d'extraction par expressions régulières, l'incrémentation de la métrique de surveillance Prometheus OCR_FAILURES, ainsi que l'insertion sécurisée des données dans la base PostgreSQL sont détaillées dans l'annexe **parsing_ocr.pdf** : Traitement de la réponse OCR et persistance en base de données à la fin de ce document."
...
![Interface d'ingestion OCR](img/img_E3/C10_ocr_ui.png)
...
![Payload JSON affichant les 9 mesures physico-chimiques extraites avec succès par le service OCR](img/img_E3/C10_ocr_payload.png)
...
Au lieu de faire crasher le conteneur API, l'application renvoie un statut de succès partiel ``HTTP 201`` (pending) à l'interface client<br>
 ...
 ##### *Interception de la panne par l'API Backend et renvoi d'un statut "pending"*
  ![Interface Swagger montrant l'interception de la panne par l'API Backend avec le renvoi d'un statut HTTP 201 pending](img/img_E5/OCR_ko_swagger.png)
...
![Interface web Flask affichant un bandeau d'avertissement jaune modéré indiquant que le service OCR est temporairement ralenti](img/img_E5/OCR_ko_front_reseau_hs.png)
...
![Interface flask - Avertissement rouge affichant le JSON brut ("Service OCR indisponible...")](img/img_E5/OCR_ko_front.png)
