...
depuis :
    ...
    - une base de données
    - un système big data
*en programmant le script* adapté afin de pérenniser la collecte des données.
...
## Projet : L'Observatoire de la Mobilité à Marseille

Pour ce bloc de compétences, j'ai conçu un pipeline de données visant à alimenter un Observatoire de la Mobilité.  Ce projet agrège des données hétérogènes issues de différents acteurs de la mobilité marseillaise :
- les trottinettes en libre-service (VOI)
- les Alertes de sécurité des clients du réseau RTM
- la mobilité maritime (Navettes maritimes)

L'objectif est de consolider ces flux dans un entrepôt de données unifié (modèle en étoile), afin de permettre des analyses croisées par zone géographique (IRIS).
...
**Tableau récapitulatif des 5 sources de données (C1) :**

| Type de Source | Provenance & Description | Format | Technologie / Librairie | Volume / Fréquence |
| :--- | :--- | :--- | :--- | :--- |
| **Web API** | **API VOI (MDS Provider)** : État des flottes de trottinettes, trajets et géolocalisation. | JSON | Python `requests` | Milliers d'événements / incrémental mensuel. |
| **Web Scraping** | **Interface Waryme (RTM)** : Alertes de sécurité sur le réseau. | HTML | Python `Playwright` | Scraping hebdomadaire. |
| **Fichier** | **Navettes Maritimes (RTM)** : Registre historique d'exploitation métier (`maritime_clean.csv`). | CSV | Python `pandas` | Fichier statique historique. |
| **Système Big Data** | **Historique VOI & RTM** : Snapshots et historiques massifs de trajets convertis pour PowerBI. | `.parquet` | Python `DuckDB` / `pandas` | Gigaoctets de données historiques. |
| **Base de Données** | **Référentiel IRIS (Marseille)** : Découpage géographique officiel de la ville utilisé pour les jointures spatiales. | SQL | PostgreSQL `sqlalchemy` | Statique (référentiel des zones). |
...
Les trajets VOI, les alertes sécurité Waryme et les données d'exploitation des navettes ont été fusionnés en utilisant la clé de voûte de l'Observatoire : le code IRIS.<br>
Une jointure spatiale rattache chaque événement à son quartier marseillais<br>
ex: `zone_iris_start_code`, `zone_iris_end_code`
